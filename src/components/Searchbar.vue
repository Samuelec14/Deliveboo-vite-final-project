...
export default {
  ...
  methods: {
    ...
    async fetchTypes() {
      this.loading = true;
      axios
        .get(`http://127.0.0.1:8000/api/type/type`)
        .then((response) => {
    ...
    navigateToDish(restaurantId) {
      this.$router.push({
        name: "dish",
        params: { restaurant_id: parseInt(restaurantId) },
      });
    },
    submitForm() {
      this.loading = true;
      ...
      if (values.length > 0) {
        axios
          .get(`http://127.0.0.1:8000/api/restaurant/results`, {
            params: {
              search: values,
            },
          })
        ...
        this.loading = true;
        axios
          .get(`http://127.0.0.1:8000/api/restaurant/restaurant`)
          .then((response) => {
...
<template>
  ...
  <h2 class="d-flex align-items-center justify-content-center mt-5">
    Cosa vuoi mangiare?
  </h2>
